...
test("Users role assignment Select keeps accessible Modal ownership", async ({ page }, testInfo) => {
  const unknown = await openWithFixtures(page, "/system-management/users");
  const row = page.getByRole("row").filter({ hasText: "Demo User" });
  ...
  await expect(roleSelect).toHaveAttribute("id", "assign-role");
  await expect(roleSelect).toHaveAttribute("aria-labelledby", "assign-role-label");
  await expect(dialog.locator('label[for="assign-role"]')).toHaveText("分配新角色");
