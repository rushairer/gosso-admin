...
const DialogContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, DialogContentProps>(
  ({ className, children, showCloseButton = true, closeLabel = 'Close', onBackdropClick, ...props }, ref) => (
    <DialogPortal>
      <div
        className="ds-dialog-portal fixed inset-0 pointer-events-auto"
        onClick={
          onBackdropClick
            ? (event) => {
...
const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn('ds-dialog-footer flex flex-col-reverse gap-2 sm:flex-row sm:justify-end', className)}
    {...props}
  />
);
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn('text-lg font-bold leading-none tracking-tight text-[var(--ds-text-primary)] text-left', className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn('text-sm leading-relaxed text-[var(--ds-text-secondary)] text-left', className)}
    {...props}
  />
));
