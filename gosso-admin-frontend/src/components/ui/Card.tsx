  ...
  return (
    <div className={cn('flex items-start justify-between gap-4 p-6', className)}>
      <div className="space-y-1.5 min-w-0 flex-1">
        {title && <h3 className="text-lg font-semibold leading-none tracking-tight text-foreground">{title}</h3>}
...
export function CardTitle({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <h3 className={cn('text-lg font-semibold leading-none tracking-tight text-foreground', className)}>{children}</h3>
  );
}
...
export function CardContent({
  ...
  return <div className={cn('p-6 pt-0', flush && 'p-0', className)}>{children}</div>;
}
